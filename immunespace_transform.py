...
def get_participants(hcc_db, immport_db, investigation_id):
    ...
    query = f'''SELECT p.*, p2a.arm_id, e.start, e.unit, e.t0_event_type, 
                   bs.curie AS biological_sex_curie, r.curie AS race_curie, 
                   eth.curie AS ethnicity_curie, geo.curie AS geolocation_curie,
                   u.curie AS unit_curie
            FROM participant p
            JOIN participant_2_arm p2a ON p.participant_id = p2a.participant_id
            LEFT JOIN event e ON p.participant_id = e.participant_id 
                             AND e.investigation_id = '{investigation_id}' 
                             AND e.t0_event_type IN {age_fields}
            LEFT JOIN biological_sex bs ON p.biological_sex = bs.label
            LEFT JOIN race r ON p.race = r.label
            LEFT JOIN ethnicity eth ON p.ethnicity = eth.label
            LEFT JOIN geolocation geo ON p.geolocation = geo.label
            LEFT JOIN unit u ON e.unit = u.label
            ORDER BY p.row_order;
            '''
    ...
    for index, row in participant_table.iterrows():
        ...
        participants.append(Participant(akc_id=format_id(investigation_id, 'participant', row['participant_id']),
                                        ...
                                        biological_sex=row['biological_sex_curie'],
                                        age=row['start'],
                                        age_unit=row['unit_curie'],
                                        age_event=row['t0_event_type'], # "Age at Study Day 0" or "Age at enrollment"
                                        race=row['race_curie'],
                                        ethnicity=row['ethnicity_curie'],
                                        geolocation=row['geolocation_curie']))
...
def get_life_events(hcc_db, investigation_id):
    ...
    query = f'''
        SELECT e.*, 
           u.curie AS unit_curie, et.curie AS event_type_curie, geo.curie AS geolocation_curie
    FROM event e
    LEFT JOIN unit u ON e.unit = u.label
    LEFT JOIN event_type et ON e.event_type = et.label
    LEFT JOIN geolocation geo ON e.geolocation = geo.label
    WHERE e.investigation_id = '{investigation_id}';
    '''

    events_table = read_db_table_from_query(hcc_db, query)
    ...
    for index, row in events_table.iterrows():
        life_events.append(LifeEvent(akc_id=format_id(investigation_id, 'event', row['event_id']),
                                     ...
                                     life_event_type=row['event_type_curie'],
                                     geolocation=row['geolocation_curie'],
                                     t0_event=row['t0_event'],
                                     t0_event_type=row['t0_event_type'],
                                     start=row['start'],
                                     duration=row['duration'],
                                     time_unit=row['unit_curie'])) # not an ontology yet, can also use 'unit' for a text-description
    return life_events
...
def get_immune_exposures(hcc_db, investigation_id):
    ...
    query = f'''SELECT ie.*, 
                       em.curie AS exposure_material_curie, d.curie AS disease_curie, ds.curie AS disease_stage_curie
                FROM immune_exposure ie
                LEFT JOIN exposure_material em ON ie.exposure_material = em.label
                LEFT JOIN disease d ON ie.disease = d.label
                LEFT JOIN disease_stage ds ON ie.disease_stage = ds.label;
                '''

    immune_exposures_table = read_db_table_from_query(hcc_db, query)
    ...
    for index, row in immune_exposures_table.iterrows():
        immune_exposures.append(ImmuneExposure(akc_id=format_id(investigation_id, 'immuneexposure', row['event_id']),
                                               ...
                                               exposure_material=row['exposure_material_curie'],
                                               disease=row['disease_curie'],
                                               disease_stage=row['disease_stage_curie'], # or use 'disease_stage' for text description, since this is an ontology in ImmuneSpace but not in AKC yet
...
def get_specimens(hcc_db, investigation_id):
    ...
    query = f'''
    SELECT s.*, st.curie AS specimen_type_curie
    FROM specimen s
    LEFT JOIN specimen_type st ON s.specimen_type = st.label;   
    '''


    specimen_table = read_db_table_from_query(hcc_db, query)
    ...
    for index, row in specimen_table.iterrows():
        specimens.append(Specimen(akc_id=format_id(investigation_id, 'specimen', row['specimen_id']),
                                  life_event=format_id(investigation_id, 'event', row['event_id']),
                                  specimen_type=row['specimen_type_curie'], # in ImmuneSpace this is an enum, in AKC it is not (yet)
                                  tissue=row['tissue'], # todo convert to curie: but ImmuneSpace has this as free text (always empty?)
                                  process=row['process'])) # todo process always empty in ImmuneSpace?
