...
docker run -v ${AIRRKB_IMPORT}:/ak_data --network ak-db-network -it postgres:16 psql ${PG_AK_CONN} -c "INSERT into "\"QueryAssay"\" SELECT json_data->>'akc_id', json_data FROM tmp_table;"
